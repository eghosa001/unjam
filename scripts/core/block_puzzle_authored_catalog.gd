extends RefCounted
class_name BlockPuzzleAuthoredCatalog

const TOTAL_LEVELS := 10000
const CHAPTER_SIZE := 500
const CHAPTER_COUNT := 20
const CATALOG_VERSION := 1
const ROOT := "res://data/block_puzzle/authored"
...
static func recipe(level_number: int) -> Dictionary:
	...
	var chapter_number := int((n - 1) / CHAPTER_SIZE) + 1
	var rows := _chapter(chapter_number)
