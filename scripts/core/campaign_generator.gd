...
static func generate(level_number: int) -> Dictionary:
	...
	return {
		...
		"milestone": milestone,
		"par_moves": clamp(2 + int(difficulty_score * 0.8), 3, 18),
		"rescue_id": rescue_id,
...
static func _campaign_tier(level_number: int) -> int:
	...
	if level_number <= 1500:
		return 2
	if level_number <= 3000:
		return 3
	if level_number <= 5000:
		return 4
	if level_number <= 7500:
		return 5
	return 6
...
static func _difficulty_score(campaign_tier: int, rhythm_label: String, milestone: String) -> int:
	...
	match rhythm_label:
		...
		"boss": rhythm_bonus = 6
		_: rhythm_bonus = 1
	var score := 1 + campaign_tier * 2 + rhythm_bonus
	if milestone == "milestone":
		score += 1
	elif milestone == "world_finale":
		score += 2
	return clamp(score, 1, 20)
...
static func _add_fillers(pieces: Array[Dictionary], size: int, center: Vector2i, seed_value: int, amount: int) -> void:
	...
	var attempts := 0
	while amount > 0 and attempts < 500:
		attempts += 1
