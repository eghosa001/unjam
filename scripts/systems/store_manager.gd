...
func purchase(product_id: String) -> bool:
	...
	if provider_ready() and provider.has_method("purchase"):
		...
		if accepted==false: purchase_in_progress=false
		return accepted!=false
	if bool(ProjectSettings.get_setting("monetization/test_mode",false)) and OS.get_name()!="Android":
		PurchaseVerifier.verify(product_id,"desktop-test",func(valid: bool, reason: String): _on_verified(product_id,"desktop-test",valid,reason)); return true
	purchase_in_progress=false; purchase_failed.emit(product_id,"Google Play Billing provider unavailable"); return false

func confirm_purchase(product_id: String, purchase_token: String="") -> void:
	if not PRODUCTS.has(product_id): purchase_in_progress=false; return
...
func _on_verified(product_id: String, token: String, valid: bool, reason: String) -> void:
	...
	if non_consumable and product_id in purchased:
		purchase_in_progress=false; purchase_succeeded.emit(product_id); return
	if product_id==PRODUCT_REMOVE_ADS: AdManager.set_remove_ads_purchased(true)
	elif product_id==PRODUCT_STARTER_PACK:
		AdManager.set_remove_ads_purchased(true); SaveManager.data.starter_pack_purchased=true; SaveManager.add_coins(coins)
	elif coins>0:
...
func _provider_purchase_failed(product_id: String, reason: String="Purchase failed") -> void:
	purchase_in_progress=false; purchase_failed.emit(product_id,reason); AnalyticsManager.track("purchase_failed",{"product":product_id,"reason":reason})
