...
static func style_display_title(label_node: Label, fill: Color, outline_color: Color = Color("#071d55"), outline_size: int = 2) -> void:
	...
	# Keep title depth without creating a second dark glyph after phone scaling.
...
static func premium_button(text_value: String, font_size: int, text_color: Color, fill: Color, radius: float, border: Color = Color.TRANSPARENT, border_width: float = 0.0) -> Button:
	...
	# Shared Figma buttons must not inherit legacy text shadows/outline from an
	# older screen theme; those effects look doubled after compact-phone scaling.
...
static func label(text_value: String, font_size: int, color: Color, bold := false) -> Label:
	...
	# Body labels stay optically crisp; display titles opt into their smaller,
	# controlled depth explicitly through style_display_title().
	result.add_theme_color_override("font_shadow_color", Color.TRANSPARENT)
	result.add_theme_color_override("font_outline_color", Color.TRANSPARENT)
	result.add_theme_constant_override("shadow_offset_x", 0)
	result.add_theme_constant_override("shadow_offset_y", 0)
