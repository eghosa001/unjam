...
func _draw() -> void:
	...
	var selected_bob: float = sin(pulse_time * 4.0) * 2.0 if is_selected else 0.0
	var lift: float = (-11.0 + selected_bob) if is_selected else -3.0 * hover_amount
	...
	for slot in range(CAPACITY):
		...
		var slot_rect: Rect2 = Rect2(Vector2(inner.position.x, y + 1), Vector2(inner.size.x, slot_h - 2))
		if slot < layers.size():
			...
			var surface_y: float = slot_rect.position.y + 4.0
	...
	var rim_color: Color = Color("67e8cf") if is_selected else Color(0.67, 0.79, 0.94, 0.48 + hover_amount * 0.24)
