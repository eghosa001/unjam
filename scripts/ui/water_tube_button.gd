...
func _draw() -> void:
	var rect := Rect2(Vector2(17, 20), size - Vector2(34, 42))
	var lift := -8.0 if is_selected else -2.0 * hover_amount
	var body := rect.translated(Vector2(0, lift))
	var shadow := body.translated(Vector2(0, 12))
	_draw_round_rect(shadow, Color(0, 0, 0, 0.28), 30.0)
	_draw_round_rect(body, Color(0.035, 0.075, 0.14, 0.96), 30.0)

	var rim_color := Color("67e8cf") if is_selected else Color(0.65, 0.77, 0.92, 0.45 + hover_amount * 0.25)
	...
	var inner := Rect2(body.position + Vector2(14, 30), body.size - Vector2(28, 58))
	var slot_h := inner.size.y / float(CAPACITY)
	for slot in range(CAPACITY):
		var y := inner.end.y - slot_h * float(slot + 1)
		var slot_rect := Rect2(Vector2(inner.position.x, y + 2), Vector2(inner.size.x, slot_h - 4))
		if slot < layers.size():
