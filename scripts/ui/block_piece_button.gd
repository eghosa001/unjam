...
func _draw() -> void:
	...
	if points.is_empty():
		return
	# A shallow pedestal makes the three tray choices read as physical,
	# draggable game pieces instead of small artwork floating on a pale panel.
	# Keep it translucent so the tray material still shows through.
