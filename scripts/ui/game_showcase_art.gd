...
func _draw_rescue_rush() -> void:
	...
	var bob: float = sin(phase * 2.2) * 7.0
	...
	for i in range(4):
		...
		var p: Vector2 = c + d * (150 + sin(phase * 1.8 + i) * 8)
		...
		var tip: Vector2 = p + d * 28
		var side: Vector2 = Vector2(-d.y, d.x) * 16
		draw_colored_polygon(PackedVector2Array([tip + d * 14, tip - d * 10 + side, tip - d * 10 - side]), accent)
	for j in range(5):
		var alpha: float = 0.16 - j * 0.025
		draw_line(c + Vector2(-250 + j*22, 190), c + Vector2(-170 + j*22, 190), Color(accent, alpha), 8)

func _draw_water_sort() -> void:
	var c: Vector2 = size * 0.5
	var colors: Array[Color] = [Color("ff6b7a"), Color("ffd166"), Color("5da9ff"), Color("67e8cf")]
	for i in range(5):
		var x: float = c.x - 220 + i * 110
		var y: float = c.y + sin(phase * 1.8 + i * 0.7) * 8
		...
		for layer in range(4):
			var liquid_y: float = y + 76 - layer * 38
			var col: Color = colors[(i + layer) % colors.size()]
			var wave: float = sin(phase * 3.0 + i + layer) * 2.5
			draw_rect(Rect2(Vector2(x - 26, liquid_y - 28 + wave), Vector2(52, 32)), Color(col, 0.92), true)
	var sx: float = c.x - 110.0
	var tx: float = c.x + 110.0
	var stream_y: float = c.y - 150.0 + sin(phase * 2.8) * 6.0
...
func _draw_block_puzzle() -> void:
	var c: Vector2 = size * 0.5
	var origin: Vector2 = c - Vector2(180, 180)
	var cell := 44.0
	for y in range(8):
		for x in range(8):
			var rect := Rect2(origin + Vector2(x, y) * cell, Vector2(cell - 5, cell - 5))
			var filled: bool = ((x + y * 3) % 7 in [0,1,3]) and not (x in [3,4] and y in [3,4])
			var col: Color = Color(accent, 0.75) if filled else Color(1,1,1,0.05)
			draw_rect(rect, col, true)
	var hover: Vector2 = origin + Vector2(3.0 * cell, 1.3 * cell) + Vector2(0, sin(phase * 2.1) * 10)
	for p: Vector2 in [Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(2,1)]:
	...
	var sweep_x: float = origin.x + fposmod(phase * 110.0, 8.0 * cell)
