...
func _draw() -> void:
	...
	match game_id:
		"water_sort":
			for i in range(5):
				var x: float = center.x - 220.0 + float(i) * 110.0
				var y: float = center.y + sin(phase * 1.6 + float(i)) * 5.0
		...
		"block_puzzle":
			var cell: float = 40.0
			var origin: Vector2 = center - Vector2(cell * 4.0, cell * 4.0)
			for y in range(8):
				for x in range(8):
					var filled: bool = ((x * 3 + y * 5) % 8) in [0,1,3]
		...
		_:
			...
			for dir: Vector2 in directions:
				var pos: Vector2 = center + dir * (150.0 + sin(phase * 1.7) * 7.0)
