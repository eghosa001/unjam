...
const DARK_TOP := Color("#242b33")
const DARK_MID := Color("#1d232a")
const DARK_BOTTOM := Color("#161b22")
...
func _build_reference_selector(canvas: Control) -> void:
	...
	var selector_subtitle := _add_text(canvas, "3 PUZZLES • 1 JOURNEY", Rect2(83, 51, 186, 30), 12, Color("#c6d9ec") if _selector_dark() else Color("#31566f"), false)
...
func _add_bottom_nav(canvas: Control) -> void:
	...
	var nav_fill := Color("#20272f") if _selector_dark() else Color("#e3e8ed")
	var nav_border := Color("#34404c") if _selector_dark() else Color("#c3cbd3")
