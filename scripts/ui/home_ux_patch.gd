...
func _process(_delta: float) -> void:
	var home := get_parent().get_node_or_null("PremiumHome") as Control
...
func _patch_layout(outer: MarginContainer) -> void:
	...
	panel.add_theme_stylebox_override("panel", _box(Color("0b1626"), 28, Color(accent, 0.42), 2))
	...
	meta.add_theme_color_override("font_color", Color("d8e4f4"))
	copy.add_child(meta)
	...
	progress.add_theme_stylebox_override("background", _box(Color("101b2d"), 7))
	...
	stats.add_theme_color_override("font_color", Color("9aa9bf"))
	row.add_child(stats)
