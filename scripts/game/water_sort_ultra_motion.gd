...
func _visual_mouth_local(control: Control) -> Vector2:
	var outer_x := control.size.x * 0.22
	var outer_y := 13.0
	var outer_w := control.size.x * 0.56
	var outer_h := control.size.y - 42.0
	var neck_h := outer_h * 0.10
	var lip_y := outer_y + neck_h * 0.40 + 3.0
	if control.rotation > 0.08:
		return Vector2(outer_x + outer_w + 3.0, lip_y)
	if control.rotation < -0.08:
		return Vector2(outer_x - 3.0, lip_y)
