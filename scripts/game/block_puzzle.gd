...
func build_ui() -> void:
	...
	pieces_title.add_theme_font_size_override("font_size", 18)
...
func load_level() -> void:
	...
	target_lines = int(config.target_lines)
	par_placements = int(config.par)
...
func undo_move() -> void:
	...
	cells = state.cells.duplicate(true)
	pieces = state.pieces.duplicate(true)
	selected_piece = int(state.selected)
	score = int(state.score)
	lines_cleared = int(state.lines)
	placements = int(state.placements)
	piece_batch = int(state.batch)
