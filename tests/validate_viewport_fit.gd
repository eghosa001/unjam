...
func _run() -> void:
	...
	# visuals. This catches regressions where a future refactor accidentally
	# reconnects the old centre-to-centre stream implementation.
	if not water_scene.contains("water_sort_casual.gd"):
		return _fail("Water Sort scene no longer uses the active casual/motion stack")
