...
func _run() -> void:
	...
	if int(save_manager.data.total_levels_completed) != 1:
		errors.append("Replay incorrectly increments unique completion count")
	if int(save_manager.data.perfect_clears) != 1:
		errors.append("Replay incorrectly increments unique perfect count")
	if int(first.get("base_coins", -1)) != 75:
		errors.append("First-clear reward accounting is incorrect")
	if int(second.get("base_coins", -1)) != 0:
		errors.append("Replay reward duplication protection failed")
	if int(save_manager.data.get("save_version", 0)) < 3:
		errors.append("Robust save version is missing")

	ads.completed_since_interstitial = 0
	ads.set_ads_enabled(true)
	for i in range(ads.interstitial_interval):
		ads.note_level_completed()
	if not ads.should_show_interstitial():
		errors.append("Interstitial pacing does not reach ready state")

	var missions: Array = retention.daily_missions()
	if missions.size() != 3:
		errors.append("Daily mission count changed unexpectedly")
