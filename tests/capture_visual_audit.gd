...
func _fast_visual_enabled(scope: String) -> bool:
	...
	if requested.is_empty():
		return true
	return scope in requested.split(",", false)

func _run_fast_visual_audit(main: Node, shell: Node) -> void:
	# PR loop: render only surfaces affected by the current change. The manual
	# Visual UI Audit still owns the complete matrix.
	...
	if _fast_visual_enabled("result"):
		var compact_result := PremiumResultOverlay.new()
		compact_result.configure(
			"LEVEL COMPLETE",
			"Clean play. Strong route. Keep the streak moving.",
