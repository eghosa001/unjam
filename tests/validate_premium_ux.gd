...
func _initialize() -> void:
	...
	_check_source("res://scripts/game/water_sort_ultra_motion.gd", ["_balanced_columns", "tube_count <= 6", "return 3"], failures)
	...
	if not failures.is_empty():
		...
		quit(1)
		return
	print("Premium UX contract validated: responsive Rescue Rush board bounds, forgiving Block Puzzle placement, physically readable and balanced Water Sort pours, stable navigation, and app-wide premium surfaces.")
	quit(0)
