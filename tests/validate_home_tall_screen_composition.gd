...
const MAX_HERO_HEIGHT := 500.0
const MIN_HERO_HEIGHT := 280.0
...
func _validate_viewport(viewport_size: Vector2i) -> bool:
	...
	if hero.size.y < MIN_HERO_HEIGHT:
		return _fail("Home hero became too small at %s: %.1fpx (min %.1fpx), size=%s" % [str(viewport_size), hero.size.y, MIN_HERO_HEIGHT, str(hero.size)])
	...
	var logical_height := maxf(1.0, home.size.y)
	var cluster_center_ratio := (cluster_rect.position.y + cluster_rect.size.y * 0.5) / logical_height
	...
	if hero_to_cluster_gap > logical_height * 0.10:
		return _fail("Home hero-to-action gap is excessive at %s: %.1fpx" % [str(viewport_size), hero_to_cluster_gap])
	print("HOME_COMPOSITION %s hero=%s action_center=%.3f gap=%.1f" % [str(viewport_size), str(hero.size), cluster_center_ratio, hero_to_cluster_gap])
