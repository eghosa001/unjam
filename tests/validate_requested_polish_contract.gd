...
func _init() -> void:
	...
	_require("res://scripts/ui/water_tube_3d_motion.gd", ["Premium bottle silhouette built from real geometry", "Stronger crystal alpha/emission", "GLASS_BODY_RADIUS := 0.72", "_glass_material_3d", "BottleShoulder3D", "BottleNeck3D", "BottleInnerWall3D"], errors)
	_require("res://scripts/game/water_sort_reference_motion.gd", ["tilt_degrees := 32.0 if MotionSystem.reduced() else 62.0", "clear downhill rim and visible arc"], errors)
