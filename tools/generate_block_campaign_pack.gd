...
func _run() -> void:
	...
	for world in range(1, Progression.WORLD_COUNT + 1):
		...
		for level in range(first, last + 1):
			...
			metadata["difficulty_floor"] = int(profile.get("difficulty_floor", 0))
			metadata["difficulty_ceiling"] = int(profile.get("difficulty_ceiling", 100))
			metadata["world"] = int(profile.get("world", world))
			metadata["chapter_global"] = int(profile.get("chapter_global", 1))
			metadata["chapter_in_world"] = int(profile.get("chapter_in_world", 1))
			metadata["milestone"] = String(profile.get("milestone", "normal"))
			metadata["objective_family"] = String(profile.get("objective", "score"))
			metadata["piece_tier"] = int(profile.get("piece_tier", 1))
			metadata["planning_horizon"] = int(profile.get("planning_horizon", 1))
			metadata["profile_initial_occupancy"] = float(profile.get("initial_occupancy", 0.0))
			metadata["move_limited"] = bool(profile.get("move_limited", false))
			var first_attempt: Vector2 = profile.get("first_attempt_target", Vector2.ZERO)
			metadata["first_attempt_target_low"] = first_attempt.x
			metadata["first_attempt_target_high"] = first_attempt.y
